...
func TestMapStrToMapstr(t *testing.T) {
	...
	d, err := mapstr.NewFromInterface(testData)
	t.Log(d)
	if err != nil {
		t.Error(err.Error())
		return
	...
	d1, err := testData2.MapStrArray("aa")
	t.Log(d1)
	if err != nil {
		t.Error(err.Error())
		return
...
func TestECS(t *testing.T) {
	...
	data.ForEach(func(key string, val interface{}) error {
		t.Log(key)
		t.Log(mapstr.NewFromInterface(val))
		return nil
	})
	t.Logf("output:%#v", data)
	t.Log(data.Get("VpcAttributes"))
	d, _ := data.Get("VpcAttributes")
	d1, _ := mapstr.NewFromInterface(d)
	t.Log(d1.Get("PrivateIpAddress"))
}
